...
BEGIN
  ...
  -- 6. Stripe webhook silence. Only alarm if a renewal was actually due during
  -- the silent period. Quiet stretches with no renewals due are normal.
  SELECT count(*) INTO m FROM public.members;
  SELECT max(created_at) INTO last_event_at FROM public.stripe_webhook_events;

  IF m > 0 AND (last_event_at IS NULL OR last_event_at < now() - interval '7 days') THEN
    SELECT count(*) INTO renewals_due
    FROM public.subscriptions s
    WHERE s.status IN ('active', 'past_due')
      AND s.current_period_end IS NOT NULL
      AND s.current_period_end > coalesce(last_event_at, '1970-01-01'::timestamptz)
      AND s.current_period_end <= now();

    IF renewals_due > 0 THEN
      issues := issues || jsonb_build_array(jsonb_build_object(
        'key', 'stripe_webhook_silence', 'severity', 'warning',
        'title', 'No Stripe webhook events received despite ' || renewals_due || ' renewal(s) being due',
        'detail', jsonb_build_object('members', m, 'renewals_due_since_last_event', renewals_due, 'last_event_at', last_event_at)));
    END IF;
  END IF;
