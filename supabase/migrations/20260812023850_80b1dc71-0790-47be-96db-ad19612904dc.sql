...
BEGIN
  ...
  -- 6. Stripe webhook silence. Quiet stretches are normal at low member counts,
  -- so only alarm after a full week with no events at all.
  SELECT count(*) INTO m FROM public.members;
  SELECT count(*) INTO n FROM public.stripe_webhook_events WHERE created_at > now() - interval '7 days';
  IF m > 0 AND n = 0 THEN
    issues := issues || jsonb_build_array(jsonb_build_object(
      'key', 'stripe_webhook_silence', 'severity', 'warning',
      'title', 'No Stripe webhook events received in the last 7 days',
