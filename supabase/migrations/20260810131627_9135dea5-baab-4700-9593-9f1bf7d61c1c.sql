...
DROP POLICY IF EXISTS "Service role manages system alerts" ON public.system_alerts;
...
BEGIN
  ...
  SELECT count(*) INTO n FROM public.email_send_log
  ...
  IF n > 0 THEN
    SELECT jsonb_agg(jsonb_build_object('template', template_name, 'to', recipient_email, 'status', status, 'error', error_message, 'at', created_at))
      INTO detail FROM (
        ...
        WHERE status IN ('dlq','failed','bounced','complained') AND created_at > now() - interval '24 hours'
